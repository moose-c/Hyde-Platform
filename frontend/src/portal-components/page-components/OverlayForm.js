...
import { yearsObject, yearValueList, indicatorNcObj, indicatorTxtObj, indicatorNcOrder } from "../../util/createData"  /* as ind.type : {ind -> Indicator Name} */
...
export default function OverlayForm({ map, setMap, setSelection, currentYear, setCurrentYear, ovIndicator, setOvIndicator, overlay, setOverlay, afterChange, setAfterChange }) {
    ...
    return (
        <>
            <Form>
                <Row>
                    <Form.Label style={{ marginTop: '3px' }}>Year {yearsObject[currentYear]} <br />
                        <div style={{ display: 'flex', justifyContent: "center" }}>
                            <RangeSlider tooltipStyle={{ display: "none" }} style={{ width: 200 }} onChange={(e) => { setAfterChange(false); setCurrentYear(yearValueList[e.target.value]) }} onAfterChange={() => setAfterChange(true)} type="range" min={0} max={yearValueList.length - 1} step={1} value={yearValueList.indexOf(currentYear)} />
                        </div>
                    </Form.Label>
                </Row>
                <Row>
